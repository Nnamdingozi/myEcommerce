...
// const Navbar: React.FC<NavBarProps> = ({ count }) => {
//   const { user, logout, token } = useUser();
//   const { products, loading } = useProduct() as { products: ProductDetails[] }; 
//   const router = useRouter();
//   const [isMenuOpen, setIsMenuOpen] = useState(false);
...
//               className='w-full h-8 py-2 pl-10 pr-4 border border-gray-300 rounded-md focus:ring-2 focus:ring-red-800 focus:outline-none ursor-pointer'
//             />
//             <div className='absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none'>
//              <MagnifyingGlassIcon
...
//         <li
//           key={product.id}
//           onClick={() => router.push(`/search?query=${encodeURIComponent(searchQuery)}`)}
