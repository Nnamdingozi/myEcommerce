...
//         createPaystackHook,
//         verifyPaystackHook,
//     } = orderHookLogic(setOrder, setUserOrder, setIsLoading, setError, setSuccessMessage, setIsVerifying); // Pass state setters

//     const createNewOrder = useCallback(
//         async (paymentMtd: string, shippingAddy: string, shippingMtd: string, curr: string): Promise<{ orderId: number } | null | undefined> => {
//             return await createNewOrderHook(token, paymentMtd, shippingAddy, shippingMtd, curr);
//         },
//         [createNewOrderHook, token]
//     );

//     const getUserOrder = useCallback(async (): Promise<Order[] | null | undefined> => {
//         return await getUserOrderHook(token);
//     }, [getUserOrderHook, token]);

//     const getUserOrderById = useCallback(async (orderId: number): Promise<Order | null | undefined> => {
//         return await getUserOrderByIdHook(token, orderId);
//     }, [getUserOrderByIdHook, token]);

//     const createPaystack = useCallback(async (orderId: number): Promise<Paystack | null | undefined> => {
//         const result = await createPaystackHook(orderId);
//         // router.push('/');
//         return result;
//     }, [createPaystackHook]);

//     const verifyPayment = useCallback(async (reference: string): Promise<VerifyTransactionResponse | null | undefined> => {
//         const result = await verifyPaystackHook(reference);
//         // router.push('/');
//         return result;
//     }, [verifyPaystackHook]);
...
//     handleCloseOrderDetails: () => void;
//     isVerifying: boolean;
//     setIsVerifying: (show: boolean) => void;
...
//         try {
//             const verifiedPay = await verifyPaystackHook(reference);
//             console.log('value return after verify pay in context:', verifiedPay);
//             return verifiedPay;
//         } catch (error) {
//             setError(error instanceof Error ? error.message : 'An error occurred while verifying payment');
//         } finally {
//             setIsLoading(false);
//             setIsVerifying(false);  // Reset after verification
//         }
//     }, []);
